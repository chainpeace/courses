...
int main(int argc, char **argv) {
	...
	printf ("Matrix multiplication: blocking\n");
	...
	***********************************************/
		...
		for(kk=0;kk<N;kk+=B){
			for(i=0;i<N;i++){
				for(j=jj;j<min(jj+B,N);j++){
					...
					x_blocking[i*N+j] = x_blocking[i*N+j] + r;
				}
			}
		}
	}
	...
	// You can Verify the result using check_result(). But You have to comment out this code block when profile using perf for exact analysis.
